...
- **이미지 자동 저장 및 붙여넣기**: 클립보드의 이미지(`Ctrl+V`)를 붙여넣거나 사진 파일을 드래그 앤 드롭하면 로컬 `images/` 폴더에 이미지를 안전하게 자동 저장하고 에디터에 삽입합니다. Wails 가상 서버 연동을 통해 로컬 경로 이미지도 완벽히 렌더링합니다.
...
- **몰입형 글쓰기 (Focus / Typewriter Mode)**: 현재 작성 중인 줄만 밝게 하이라이트하는 포커스 모드와, 커서를 항상 화면 중앙에 고정시켜주는 타자기 모드를 지원하여 글쓰기 몰입도를 극대화합니다.
