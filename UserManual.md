...
- **이미지 및 텍스트 붙여넣기**: 클립보드에 복사된 이미지나 텍스트를 `Ctrl + V`로 붙여넣습니다. 이미지를 붙여넣으면 로컬 폴더에 이미지를 자동 저장하고 삽입합니다. (※ 이미지 자동 저장은 문서를 1회 이상 저장한 상태에서만 지원됩니다. ※ 보안상의 이유로 **마우스 우클릭을 통한 붙여넣기 메뉴는 지원하지 않으므로 반드시 단축키(`Ctrl + V`)를 사용**해 주세요.)
...
- **포커스 모드 토글**: `Ctrl + Shift + F`
- **타자기 모드 토글**: `Ctrl + Shift + T`
...
### 3.8 포커스 모드 & 타자기 모드 (몰입형 글쓰기)
글쓰기 자체에 깊이 몰입하고 싶을 때 활용할 수 있는 두 가지 특별한 모드를 제공합니다. 에디터 툴바의 **[🎯 Focus]** 및 **[⌨️ Typewriter]** 버튼을 클릭하거나 전용 단축키를 통해 언제든지 켜고 끌 수 있습니다.
- **포커스 모드 (`Ctrl + Shift + F`)**: 현재 커서가 위치한 줄(Line)만 선명하게 보여주고, 나머지 텍스트는 반투명하게 흐리게 처리하여 오직 '지금 쓰고 있는 문장'에만 집중할 수 있게 해줍니다.
- **타자기 모드 (`Ctrl + Shift + T`)**: 문서가 길어지면 커서가 화면 맨 아래로 내려가 시선이 떨어지는 문제를 해결합니다. 타자기처럼 커서 위치를 항상 화면 정중앙에 고정하고 배경 종이가 위로 스크롤되도록 하여 목의 피로를 덜어줍니다.
